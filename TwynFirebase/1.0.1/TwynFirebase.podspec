...
Pod::Spec.new do |s|
  s.name             = 'TwynFirebase'
  s.version          = '1.0.1'
  s.summary          = 'Firebase delivery for Twyn.'
  s.description      = <<-DESC
The adapter that delivers Twyn events to Firebase Analytics. This is the only
part of the SDK that references the analytics dependency, which is why it is a
separate pod: taking Twyn does not pull Firebase into your build.
  DESC
  ...
  s.vendored_frameworks = 'TwynFirebase/TwynFirebase.xcframework'

  # Required, not optional: FirebaseAnalytics vendors a static xcframework, and CocoaPods
  # refuses to build a dynamic framework whose transitive graph contains statically linked
  # binaries. Without this, `pod lib lint` and any host using dynamic frameworks fail with
  # "target has transitive dependencies that include statically linked binaries".
  # VTMonetSDKBinary declares the same flag for the same reason.
  s.static_framework = true

  s.dependency 'Twyn', s.version.to_s
  # The fleet runs three Firebase bands (12.6.x, 12.12.x, 12.19.x), all pinned by
  # VTMonetSDKBinary. A range rather than a floor, so this pod intersects with any of
  # them instead of forcing an upgrade the ads SDK has not made.
  s.dependency 'FirebaseAnalytics', '>= 12.6', '< 13.0'
end
